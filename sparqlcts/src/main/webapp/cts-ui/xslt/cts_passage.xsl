...
<xsl:stylesheet version="2.0" xmlns:cts="http://chs.harvard.edu/xmlns/cts"
	...
	<xsl:template match="/">
		...
		<xsl:variable name="catVar">@ctsapi@?request=<xsl:value-of select="//cts:request/cts:requestName"/>&amp;stylesheet=cts_cat&amp;urn=<xsl:value-of
	...
	<xsl:template match="cts:node">
		<xsl:if test="preceding-sibling::*[1]/@parent != ./@parent ">
			<span style="display: block; width: 100%; margin-top: 0.5em; margin-bottom: 0.5em;"/>
		</xsl:if>
				...
				<xsl:when test="tei:w">
					<xsl:choose>
						<xsl:when test="preceding-sibling::*[1]/@parent != ./@parent ">
							<xsl:attribute name="class">cts_node_newline cts_node cts_inline</xsl:attribute>
						</xsl:when>
						<xsl:otherwise>
